...
//}
        ...
        stage('Deploy to GCP VM') {
            steps {
                sshagent(['tomcatkey']) {
                sh '''
                ssh -o StrictHostKeyChecking=no abuabdillah5444@34.132.231.218 "sudo docker pull aatikah/vul-djangoapp:v1 && sudo docker run -d -p 8001:8000 aatikah/vul-djangoapp:v1"
                '''
    }
    }
}
            ...
            steps {
                script {
                   ...
                   // sh 'sudo docker cp zap:/zap/zap_report.html ./zap_report.html'
                    ...
                    sh '''
                    source archerysec_env/bin/activate
